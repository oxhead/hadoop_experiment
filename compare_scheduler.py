...
def run(output_directory, model):
	...
	job_list = ["terasort", "wordcount", "grep", "nocomputation", "custommap4", "custommap6"]
	#job_size_list = ["64MB", "128MB", "256MB", "512MB", "1GB", "2GB", "4GB"]
	job_size_list = ["256MB", "1GB"]
	num_jobs = 40
	period = 20

	job_submit_timeline = job_factory.create_jobs(num_jobs=num_jobs, job_list=job_list, job_size_list=job_size_list, period=period)
	#for key in sorted(job_submit_timeline.keys()):
	#	print key, job_submit_timeline[key]
